...
function Get-KeyVaultStorePath {
  # Override wins if set (e.g. $env:KEYVAULT_STORE_PATH = "C:\Cevict_Vault\env-store.json").
  $override = [string]$env:KEYVAULT_STORE_PATH
  if ($override) { return $override }

  # Prefer Cevict_Vault when it exists (that's where the real keys live); else repo vault.
  $candidates = @(
...
function Sync-KeyVaultEnvFromManifest {
  ...
  foreach ($out in @($manifest.outputs)) {
    ...
    $lines.Add("# Updated: $((Get-Date).ToUniversalTime().ToString('yyyy-MM-dd HH:mm:ss')) UTC")
    $lines.Add("")
    ...
    foreach ($k in $keys) {
      ...
      # Force single-line: strip newlines and trim; remove one layer of surrounding double-quotes if present (avoids store typos like trailing ")
      $valStr = [string]$val
      $safe = $valStr -replace "[\r\n]+", ''
      $safe = $safe.Trim()
      if ($safe.Length -ge 2 -and $safe.StartsWith('"') -and $safe.EndsWith('"')) {
