 ...
 * Table: praxis_subscriptions (or subscriptions if PRAXIS_SUBSCRIPTIONS_TABLE not set)
 * Uses praxis_subscriptions when pro DB has subscriptions with user_id=uuid for other apps.
 */
...
const TABLE_NAME = process.env.PRAXIS_SUBSCRIPTIONS_TABLE || 'praxis_subscriptions';
...
export async function upsertSubscription(row: SubscriptionRow): Promise<void> {
  ...
  if (supabase) {
    await (supabase as SupabaseClient).from(TABLE_NAME).upsert(row, {
      onConflict: 'user_id',
    });
  } else {
...
export async function deleteSubscription(userId: string): Promise<void> {
  const supabase = await getSupabaseClient();
  if (supabase) {
    await (supabase as SupabaseClient).from(TABLE_NAME).delete().eq('user_id', userId);
  } else {
