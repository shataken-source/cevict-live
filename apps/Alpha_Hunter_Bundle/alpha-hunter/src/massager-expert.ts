...
[{
  ...
  "potentialValue": "low|medium|high"
      ...
      try {
        const discoveries = JSON.parse(jsonStr);
        ...
        // Try parsing the original match without cleanup
        try {
          ...
          console.log(`   ⚠️ AI discovery failed: ${parseErr2.message?.substring(0, 50) || 'Parse error'}`);
          return this.heuristicDiscovery(dataset);
        }
      }
    } catch (err: any) {
      console.log(`   ⚠️ AI discovery failed: ${err.message?.substring(0, 50) || 'Unknown error'}`);
      return this.heuristicDiscovery(dataset);
    }
  }
