...
async function testRun() {
  ...
  const funds = new UnifiedFundManager(kalshi);
  ...
  const kalshiMarketsList = await kalshi.getMarkets();
  console.log(`Kalshi markets: ${kalshiMarketsList.length}`);
  
  const marketsWithEdge = kalshiMarketsList.filter((m: any) => m.edge && m.edge >= 5);
  console.log(`Kalshi opportunities (5%+ edge): ${marketsWithEdge.length}`);

  if (marketsWithEdge.length > 0) {
    console.log(`Top opportunity: ${marketsWithEdge[0].title}`);
    console.log(`Edge: +${marketsWithEdge[0].edge.toFixed(1)}%`);
  }
