...
const SCORES_MS = 10000;    // live scores from ESPN every 10s
const PICKS_REFRESH_MS = 120000; // full picks reload every 2min (non-ESPN)
...
const state = {
  ...
  ws: null, picksTimer: null, scoresTimer: null, picksRefreshTimer: null,
  kalshiPrices: {}, // ticker -> last known price
  kalshiTimer: null,
  myBets: [], myBetsSummary: null, myBetsTimer: null,
  apiDate: null, lastBetsRefresh: null
};

// Sample data so you can see what the wallboard looks like when populated
function injectSampleData() {
  const today = new Date().toISOString().split('T')[0];
  state.apiDate = today;
  state.lastBetsRefresh = new Date();

  state.myBets = [
    { away_team: 'Lakers', home_team: 'Celtics', pick: 'Celtics', result: 'win', stake_cents: 600, profit_cents: 545, sport: 'NBA', confidence: 72 },
    { away_team: 'Bruins', home_team: 'Rangers', pick: 'Bruins', result: 'loss', stake_cents: 500, profit_cents: -500, sport: 'NHL', confidence: 65 },
    { away_team: 'Cowboys', home_team: 'Eagles', pick: 'Eagles', result: null, stake_cents: 600, sport: 'NFL', confidence: 68 },
    { away_team: 'Duke', home_team: 'UNC', pick: 'Duke', result: 'win', stake_cents: 550, profit_cents: 500, sport: 'NCAAB', confidence: 70 },
    { away_team: 'Yankees', home_team: 'Red Sox', pick: 'Yankees', result: 'win', stake_cents: 500, profit_cents: 450, sport: 'MLB', confidence: 66 },
    { away_team: 'Air Force', home_team: 'Wyoming', pick: 'Wyoming', result: null, stake_cents: 500, sport: 'NCAAB', confidence: 62 }
  ];

  state.myBetsSummary = {
    total: state.myBets.length,
    wins: 3,
    losses: 1,
    pending: 2,
    totalStakeCents: 3250,
    totalProfitCents: 995,
    winRate: 75,
    roi: 30.6
  };
  ...
  state.games = [
    { id: 'sample_1', away: 'LAL', home: 'BOS', awayScore: 98, homeScore: 104, period: 'Q4 2:34', clock: '7:30 PM', status: 'LIVE', sport: 'NBA', edge: '5.2', modelProb: '58.0', marketProb: '52.8', confidence: 58, pick: 'Celtics', pickIsHome: true, winPct: 62, stake: 6, gameTime: new Date(), result: null, _dryRun: false },
    { id: 'sample_2', away: 'NYI', home: 'CBJ', awayScore: 2, homeScore: 3, period: '3rd 8:12', clock: '8:00 PM', status: 'LIVE', sport: 'NHL', edge: '3.1', modelProb: '55.0', marketProb: '51.9', confidence: 55, pick: 'Blue Jackets', pickIsHome: true, winPct: 54, stake: 5, gameTime: new Date(), result: null, _dryRun: false },
    { id: 'sample_3', away: 'DAL', home: 'PHI', awayScore: 0, homeScore: 0, period: '—', clock: 'Sun 1:00 PM', status: 'UPCOMING', sport: 'NFL', edge: '4.0', modelProb: '56.0', marketProb: '52.0', confidence: 56, pick: 'Eagles', pickIsHome: true, winPct: 56, stake: 6, gameTime: new Date(Date.now() + 86400000), result: null, _dryRun: false },
    { id: 'sample_4', away: 'GSW', home: 'MIA', awayScore: 112, homeScore: 108, period: 'FINAL', clock: 'Final', status: 'FINAL', sport: 'NBA', edge: '6.0', modelProb: '62.0', marketProb: '56.0', confidence: 62, pick: 'Warriors', pickIsHome: false, winPct: 100, stake: 5, gameTime: new Date(), result: 'WIN', _dryRun: false }
  ];

  state.selectedGame = state.games[0];
  const refreshEl = document.getElementById('dataRefreshInfo');
  if (refreshEl) {
    refreshEl.textContent = 'Data ' + state.apiDate + ' · ' + state.lastBetsRefresh.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false });
    refreshEl.title = 'Sample data (real data overwrites when API loads)';
  }
  renderStats();
  renderMyBets();
  renderBetsScrollBar();
...
function getGameDateCst(gameTime) {
  ...
  const d = gameTime instanceof Date ? gameTime : new Date(gameTime);
...
async function loadPicks() {
  ...
  state.picks = raw;
  const allGames = transformPicks(raw);
  // Only show picks for today (CST) — hide tomorrow's or yesterday's games
  state.games = allGames.filter((g) => {
    const gameDate = getGameDateCst(g.gameTime);
    if (!gameDate) return true; // no date = keep (e.g. TBD)
    return gameDate === todayCst;
  });
  if (state.games.length < allGames.length) {
    console.log('[Picks] Filtered by date: ' + state.games.length + ' for ' + todayCst + ' (dropped ' + (allGames.length - state.games.length) + ' for other days)');
  }
...
// ============================================
// REALISTIC WIN PROBABILITY (score + time + sport)
// ============================================
// Parses ESPN period string and returns fraction of game remaining (0 = over, 1 = just started).
function getTimeLeftFraction(periodStr, sport) {
  if (!periodStr || periodStr === '--') return 0.5;
  const s = (periodStr + '').toLowerCase();
  const totalMin = { NBA: 48, NCAAB: 40, NCAA: 40, CBB: 40, NFL: 60, NHL: 60, MLB: 162 }[sport] || 48;
  if (s.includes('final') || s.includes('final/ot')) return 0;
  if (s.includes('halftime') || s.includes('half')) return 0.5;
  const periodMatch = s.match(/(\d{1,2}):(\d{2})\s*[-–]\s*(1st|2nd|3rd|4th|1|2|3|4|ot)/i);
  if (periodMatch) {
    const min = parseInt(periodMatch[1], 10) || 0;
    const sec = parseInt(periodMatch[2], 10) || 0;
    const periodNum = { '1st': 1, '2nd': 2, '3rd': 3, '4th': 4, '1': 1, '2': 2, '3': 3, '4': 4 }[periodMatch[3].toLowerCase()] || 1;
    const periodLength = sport === 'NHL' ? 20 : sport === 'NFL' ? 15 : sport === 'NCAAB' || sport === 'NCAA' || sport === 'CBB' ? 20 : 12;
    const minLeftInPeriod = min + sec / 60;
    const periodsLeft = (sport === 'NBA' || sport === 'NFL' ? 4 : sport === 'NHL' ? 3 : 2) - periodNum;
    const totalMinLeft = minLeftInPeriod + periodsLeft * periodLength;
    return Math.max(0, Math.min(1, totalMinLeft / totalMin));
  }
  const endMatch = s.match(/end of (1st|2nd|3rd|4th|1|2|3|4)/i);
  if (endMatch) {
    const p = { '1st': 1, '2nd': 2, '3rd': 3, '4th': 4, '1': 1, '2': 2, '3': 3, '4': 4 }[endMatch[1].toLowerCase()] || 1;
    const totalPeriods = sport === 'NHL' ? 3 : sport === 'NCAAB' || sport === 'NCAA' || sport === 'CBB' ? 2 : 4;
    return 1 - (p / totalPeriods);
  }
  return 0.5;
}

// Logistic-style curve: point diff and time left → win prob 0–100.
// When time is low, same point diff has bigger impact (face reality when down big late).
function computeRealisticWinProb(game) {
  const pickScore = game.pickIsHome ? game.homeScore : game.awayScore;
  const oppScore = game.pickIsHome ? game.awayScore : game.homeScore;
  const pointDiff = pickScore - oppScore;
  const timeLeft = getTimeLeftFraction(game.period, game.sport);

  if (game.status === 'FINAL') {
    return pointDiff > 0 ? 100 : pointDiff < 0 ? 0 : 50;
  }
  if (game.status === 'UPCOMING') {
    return Math.round(parseFloat(game.modelProb) || 50);
  }

  // With little time left, point diff dominates. With lots of time, curve is flatter.
  // "Volatility" = how many points one might reasonably make up in remaining time (sport-rough).
  const volatility = (game.sport === 'NBA' || game.sport === 'NCAAB' || game.sport === 'NCAA' || game.sport === 'CBB')
    ? 0.4 * Math.sqrt(timeLeft * 48)  // ~0.4 pts/min * sqrt(min left)
    : game.sport === 'NHL' ? 0.5 * Math.sqrt(timeLeft * 60) : 0.35 * Math.sqrt(timeLeft * 60);
  const scale = Math.max(2, volatility);
  const z = pointDiff / scale;
  const logistic = 1 / (1 + Math.exp(-z));
  const formulaWp = Math.round(100 * logistic);

  return Math.max(0, Math.min(100, formulaWp));
}
...
async function loadLiveScores() {
  ...
  for (const sport of sports) {
    ...
    try {
      ...
      for (const event of (data.events || [])) {
        ...
        if (isFinal) {
          game.winPct = computeRealisticWinProb(game);
        }
        if (isLive) {
          // Update win probability on every refresh from score + time formula
          game.winPct = computeRealisticWinProb(game);
  ...
  // Use ESPN-derived W-L and P&L when we have no settled bets from API (so wallboard isn't stuck at 0-0)
  const espnWins = state.games.filter(g => g.result === 'WIN').length;
  const espnLosses = state.games.filter(g => g.result === 'LOSS').length;
  const settledFromApi = state.myBetsSummary ? (state.myBetsSummary.wins + state.myBetsSummary.losses) : 0;
  if (settledFromApi === 0) {
    state.wins = espnWins;
...
async function loadStats() {
  try {
    const res = await fetch(API + '/progno/predictions/stats', { cache: 'no-store' });
    if (!res.ok) return;
    const data = await res.json();
    if (data.stats) {
...
function startPolling() {
  ...
  // Stats every REFRESH_MS
  state.picksTimer = setInterval(async () => {
    await loadStats();
  }, REFRESH_MS);

  // Full picks reload every 90s so new games and data refresh
  state.picksRefreshTimer = setInterval(async () => {
    const selectedId = state.selectedGame?.id;
    await loadPicks();
    if (selectedId && state.games.length) {
      const still = state.games.find(g => g.id === selectedId);
      if (still) selectGame(still);
    }
  }, PICKS_REFRESH_MS);
...
function renderStats() {
  ...
  // Use actual bets data when available; total stake and bet count always from API when we have sum
  if (sum) {
    const staked = sum.totalStakeCents / 100;
    const settled = sum.wins + sum.losses;
    const pending = sum.pending != null ? sum.pending : 0;

    setText('exposure', '$' + staked.toFixed(2));
    setText('totalBetsCount', sum.total);

    // When API has no settled results, use ESPN-derived W-L and P/L so something shows besides stake/count
    if (settled > 0) {
      state.wins = sum.wins;
      state.losses = sum.losses;
      state.todayPnl = sum.totalProfitCents / 100;
      const pnl = state.todayPnl;
      const winRatePct = Math.round((sum.wins / settled) * 100);
      setText('statRecord', sum.wins + '-' + sum.losses + (pending > 0 ? ' (' + pending + 'P)' : ''));
      setText('todayPnl', (pnl >= 0 ? '+$' : '-$') + Math.abs(pnl).toFixed(2));
      setText('winRate', winRatePct + '%');
      const pnlEl = document.getElementById('todayPnl');
      if (pnlEl) pnlEl.style.color = pnl >= 0 ? 'var(--green)' : 'var(--red)';
      if (typeof sum.roi === 'number' && !Number.isNaN(sum.roi)) {
        setText('statROI', (sum.roi >= 0 ? '+' : '') + sum.roi.toFixed(1) + '%');
      } else {
        const roi = staked > 0 ? (pnl / staked) * 100 : 0;
        setText('statROI', (roi >= 0 ? '+' : '') + roi.toFixed(1) + '%');
      }
    } else {
      // No settled bets from API — show ESPN-derived record/P/L/ROI when we have games (so more than stake/count shows)
      const w = state.wins;
      const l = state.losses;
      const pnl = state.todayPnl;
      const winRatePct = (w + l) > 0 ? Math.round((w / (w + l)) * 100) : 0;
      setText('statRecord', w + '-' + l + (pending > 0 ? ' (' + pending + 'P)' : ''));
      setText('todayPnl', (pnl >= 0 ? '+$' : '-$') + Math.abs(pnl).toFixed(2));
      setText('winRate', (w + l) > 0 ? winRatePct + '%' : (sum.total > 0 ? '—' : '--%'));
      const pnlEl = document.getElementById('todayPnl');
      if (pnlEl) pnlEl.style.color = pnl >= 0 ? 'var(--green)' : 'var(--red)';
      const roi = staked > 0 ? (pnl / staked) * 100 : state.roi;
      setText('statROI', (roi >= 0 ? '+' : '') + roi.toFixed(1) + '%');
    }
    ...
    setText('exposure', totalStake > 0 ? '$' + totalStake.toLocaleString() : '$0');
    setText('totalBetsCount', state.myBets && state.myBets.length > 0 ? state.myBets.length : '—');
...
function renderEdgePanel(g) {
  ...
  if (badge) {
    badge.textContent = (g.tier && g.tier.toUpperCase()) || (edgeNum >= 10 ? 'VALUE' : edgeNum > 0 ? 'EDGE' : '—');
...
const intelData = [
  { title: 'MODEL STATUS', rows: [{ key: 'ENGINE', val: 'Progno v3' }, { key: 'WIN RATE', val: '--%' }, { key: 'BEST EDGE', val: '--%' }] },
  { title: 'SHARP ACTION', rows: [{ key: 'MOVEMENT', val: 'Monitoring...' }, { key: 'STEAM', val: 'None detected' }] },
  { title: 'AT A GLANCE', rows: [{ key: 'STAKED', val: '$0' }, { key: 'PENDING', val: '0' }, { key: 'LIVE', val: '0' }] },
];
...
function updateIntelData() {
  const winRate = (state.wins + state.losses) > 0
    ? Math.round(state.wins / (state.wins + state.losses) * 100) + '%' : '--%';
  const bestEdge = state.games.length > 0
    ? (state.games.reduce((best, g) => Math.max(best, parseFloat(g.edge) || 0), 0)).toFixed(1)
    : '--';
  intelData[0].rows[1].val = winRate;
  intelData[0].rows[2].val = bestEdge !== '--' ? ((parseFloat(bestEdge) > 0 ? '+' : '') + bestEdge + '%') : '--%';
  const sum = state.myBetsSummary;
  const staked = sum ? '$' + (sum.totalStakeCents / 100).toFixed(2) : '$0';
  const pending = sum ? sum.pending : state.games.filter(g => g.status === 'UPCOMING' || g.status === 'LIVE').length;
  const live = state.games.filter(g => g.status === 'LIVE').length;
  intelData[2].rows[0].val = staked;
  intelData[2].rows[1].val = String(pending);
  intelData[2].rows[2].val = live > 0 ? live + ' live' : '0';
}
...
async function pollKalshiPrices() {
  ...
  for (const g of kalshiPicks) {
    try {
      const res = await fetch(API + '/progno/kalshi/market?ticker=' + encodeURIComponent(g._ticker), { cache: 'no-store' });
      if (!res.ok) continue;
      const data = await res.json();
...
async function loadMyBets() {
  const refreshEl = document.getElementById('dataRefreshInfo');
  try {
    const res = await fetch(API + '/progno/wallboard/bets', { cache: 'no-store' });
    if (!res.ok) {
      if (refreshEl) refreshEl.textContent = 'API ' + res.status;
      return;
    }
    const data = await res.json();
    if (!data.success) {
      if (refreshEl) refreshEl.textContent = 'API error';
      return;
    }
    state.myBets = data.bets || [];
    state.myBetsSummary = data.summary || null;
    state.apiDate = data.date || null;
    state.lastBetsRefresh = new Date();
    renderMyBets();
    renderStats(); // refresh BANKROLL + PERFORMANCE from actual_bets so personal bets show immediately
    if (refreshEl) {
      const dateStr = state.apiDate || '—';
      const timeStr = state.lastBetsRefresh.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false });
      refreshEl.textContent = 'Data ' + dateStr + ' · ' + timeStr;
      refreshEl.title = 'Bets API date (CST) and last refresh. Refreshes every 60s.';
    }
  } catch (e) {
    console.warn('[MyBets]', e.message);
    if (refreshEl) refreshEl.textContent = 'Fetch failed';
  }
}
...
function renderMyBets() {
  ...
  // Record badge (W-L-P); use ESPN-derived when API has no settled results
  const recEl = document.getElementById('myBetsRecord');
  if (recEl) {
    const settled = sum.wins + sum.losses;
    const w = settled === 0 ? state.wins : sum.wins;
    const l = settled === 0 ? state.losses : sum.losses;
    const p = settled === 0 ? state.games.filter(g => g.status === 'UPCOMING' || g.status === 'LIVE').length : sum.pending;
    recEl.textContent = w + 'W-' + l + 'L-' + p + 'P';
    recEl.style.color = w > l ? 'var(--green)' : l > w ? 'var(--red)' : 'var(--cyan)';
  }
...
function renderBetsScrollBar() {
  const container = document.getElementById('betsScrollInner');
  if (!container) return;
...
function setupBetsScrollBarWheel() {
  const bar = document.querySelector('.bets-scroll-bar');
  if (!bar) return;
  const scrollStep = 680;
  bar.addEventListener('wheel', (e) => {
    if (e.deltaX !== 0) return; // already horizontal scroll
    const maxScroll = bar.scrollWidth - bar.clientWidth;
    if (maxScroll <= 0) return;
    e.preventDefault();
    bar.scrollLeft += e.deltaY > 0 ? scrollStep : -scrollStep;
  }, { passive: false });
}
