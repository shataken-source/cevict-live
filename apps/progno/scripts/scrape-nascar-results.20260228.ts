 ...
 * Uses CevictScraper (cevict-scraper) to fetch historical NASCAR Cup Series race results.
...
export async function scrapeNASCARRaceResults(year: number = 2024): Promise<NASCARResult[]> {
  if (!CEVICT_SCRAPER_URL) {
    console.warn('CEVICT_SCRAPER_URL not set, using sample data');
  ...
  try {
    const scheduleHtml = await scrapeWithCevictScraper(scheduleUrl);
    const raceLinks = extractRaceLinks(scheduleHtml, year);

    console.log(`Found ${raceLinks.length} races for ${year}`);

    for (const raceUrl of raceLinks.slice(0, 5)) {
      try {
        await new Promise(r => setTimeout(r, 2000));
        const raceHtml = await scrapeWithCevictScraper(raceUrl);
...
async function scrapeWithCevictScraper(url: string): Promise<string> {
  ...
  const response = await fetch(`${CEVICT_SCRAPER_URL.replace(/\/$/, '')}/scrape`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ url, waitFor: 2000, timeout: 15000 }),
    signal: AbortSignal.timeout(20000),
  });
  if (!response.ok) throw new Error(`CevictScraper error: ${response.status} ${response.statusText}`);
