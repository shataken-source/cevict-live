/**
 * Historical Odds Scraper using CevictScraper
...
/**
 * Scrape odds using CevictScraper
 */
async function scrapeWithCevictScraper(url: string, _renderJs: boolean = true): Promise<string> {
  if (!CEVICT_SCRAPER_URL) throw new Error('CEVICT_SCRAPER_URL not set');
  ...
  const response = await fetch(`${CEVICT_SCRAPER_URL.replace(/\/$/, '')}/scrape`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ url, waitFor: 5000, timeout: 30000 }),
...
export async function scrapeNFLOdds2024(): Promise<OddsData[]> {
  ...
  try {
    ...
    const html = await scrapeWithCevictScraper(url, true);
...
async function main() {
  ...
  console.log('\n🔥 CEVICT-SCRAPER HISTORICAL ODDS SCRAPER');
