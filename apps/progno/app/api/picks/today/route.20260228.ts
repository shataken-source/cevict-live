...
import { LineMovementTracker } from '@/app/lib/line-movement-tracker'
import { InjuryImpactAnalyzer } from '@/app/lib/injury-impact-analyzer'
import { WeatherImpactAnalysisService } from '@/app/lib/weather-impact-service'
import { ArbitrageDetector, ArbitrageOpportunity } from '@/app/lib/arbitrage-detector'
import { LiveBettingMonitor } from '@/app/lib/live-betting-monitor'
import { ParlayBuilder } from '@/app/lib/parlay-builder'
...
const LEAGUE_CONFIDENCE_FLOOR: Record<string, number> = {
  ...
  cbb: PROGNO_FLOOR_CBB,        // College Baseball (separate from NCAAB)
...
async function buildPickFromRawGame(game: any, sport: string): Promise<any> {
  ...
  // Fetch betting splits data (public vs sharp money) via CevictScraper
  let bettingSplitsData = null
  const cevictScraperUrl = process.env.CEVICT_SCRAPER_URL || 'http://localhost:3009'
  if (cevictScraperUrl) {
    try {
      const splitsMonitor = new BettingSplitsMonitor(cevictScraperUrl, process.env.CEVICT_SCRAPER_API_KEY || process.env.SCRAPER_API_KEY)
