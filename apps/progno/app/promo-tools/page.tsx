  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/subscribe', {
        ...
        body: JSON.stringify({ email, source: 'promo-tools' })
      });

      if (response.ok) {
        setEmailStatus('success');
        setEmailMessage('✅ You\'re subscribed! Check your email for free picks.');
        setEmail('');
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-white p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <Card className="bg-gradient-to-r from-emerald-900/50 to-cyan-900/50 border-emerald-500/30 mb-8">
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row items-center gap-4">
              <div className="flex-1">
                <h2 className="text-xl font-bold text-white mb-1">
                  📧 Get FREE Picks & Promo Alerts
                </h2>
                <p className="text-slate-400 text-sm">
                  Subscribe for daily +EV picks, arbitrage alerts, and exclusive promo conversions
                </p>
              </div>
            ...
            {emailStatus === 'success' && (
              <p className="text-emerald-400 text-sm mt-3">{emailMessage}</p>
            )}
            {emailStatus === 'error' && (
              <p className="text-red-400 text-sm mt-3">{emailMessage}</p>
            )}
          </CardContent>
        </Card>
