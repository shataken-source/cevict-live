...
export default function SingleGamePage() {
  ...
  const [fillingLines, setFillingLines] = useState(false);
  ...
  async function fillLinesFromOdds() {
    if (!game.homeTeam || !game.awayTeam) {
      setFillMessage("Enter Home and Away team names first.");
      return;
    }

    setFillMessage("");
    setFillingLines(true);
    try {
      const sport = (game.sport || "NFL").toLowerCase();
      const gameId = `${sport}-${normalizeTeamName(game.awayTeam)}-${normalizeTeamName(game.homeTeam)}`;
      const res = await fetch(`/api/progno/v2?action=odds&gameId=${encodeURIComponent(gameId)}`);
      const json = await res.json().catch(() => ({}));

      if (!res.ok) {
        const msg = json?.error?.message || json?.error || `Failed to fetch odds (HTTP ${res.status})`;
        setFillMessage(msg);
        return;
      }
      ...
      if (!odds) {
        setFillMessage("No odds returned.");
        return;
      }
      ...
      const updated: Game = {
        ...game,
        odds: {
          home: odds.moneyline?.home ?? game.odds.home,
          away: odds.moneyline?.away ?? game.odds.away,
          spread: odds.spread?.home ?? game.odds.spread,
          total: odds.total?.line ?? game.odds.total,
        },
      };

      setHomeOddsInput(String(updated.odds.home));
      setAwayOddsInput(String(updated.odds.away));
      setGame(updated);
      predictGame(updated).then(setPrediction);
      setSimulationResults(null);
      setFillMessage("Lines filled from odds.");
    } catch (err: any) {
      setFillMessage(err?.message || "Failed to fetch odds.");
    } finally {
      setFillingLines(false);
    }
  }
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="space-y-3 border rounded p-3">
          ...
          <div>Risk: {prediction.riskLevel ?? 'n/a'} | Stake suggestion: ${prediction.stake ?? 0}</div>
          <ConfidenceGauge confidence={simulationResults ? simulationResults.avgConfidence : (prediction.confidence ?? 0)} />
