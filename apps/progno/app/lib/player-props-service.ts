...
export class PlayerPropsService {
  ...
  private async fetchPlayerStats(
    ...
    try {
      ...
      return {
        player,
        seasonAverage: parseFloat(String(playerData.seasonAvg || 0)),
        last5Average: parseFloat(String(playerData.last5Avg || playerData.recentAvg || 0)),
        homeAwaySplit: {
          home: parseFloat(String(playerData.homeAvg || playerData.seasonAvg || 0)),
          away: parseFloat(String(playerData.awayAvg || playerData.seasonAvg || 0)),
        },
        vsOpponentHistory: (playerData.vsOpponent || []).map((s: any) => parseFloat(String(s))),
        recentForm: this.assessForm((playerData.recentGames || []).map((v: any) => Number(v))),
      };
    } catch (error) {
