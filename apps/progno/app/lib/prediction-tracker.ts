...
class PredictionTracker {
  ...
  getAccuracyMetrics(): AccuracyMetrics {
    ...
    const baseMetrics: AccuracyMetrics = {
      ...
      avgConfidence: n > 0 ? completedPredictions.reduce((sum, p) => sum + p.prediction.confidence, 0) / n : 0,
      avgEdge: n > 0 ? completedPredictions.reduce((sum, p) => sum + p.prediction.edge, 0) / n : 0,
      totalProfit,
