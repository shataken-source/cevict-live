...
export default function PrognoDashboard() {
  ...
  const loadTodayBestBets = () => {
    // Use ALL cached predictions across leagues
    const allCached = Array.from(predictionCache.entries()).map(([id, pred]) => ({
      id,
      prediction: pred,
      edge: pred.edge || 0
    }));

    const withEdge = allCached.filter(item => item.edge > 0);

    withEdge.sort((a, b) => b.edge - a.edge);

    // Minimal game object for display (expand with real data if available)
    const todayBetsData = withEdge.slice(0, 10).map(item => ({
      id: item.id,
      homeTeam: "Home Team (cached)", // placeholder - can improve with game lookup
      awayTeam: "Away Team",
      league: "Cached",
      prediction: item.prediction
    }));

    setTodayBets(todayBetsData);
  };
                ...
                todayBets.map(bet => {
                  ...
                  return (
                    <div key={bet.id} className="space-y-6">
                      ...
                      <EnhancedPicksCard
                        pick={{
                          sport: bet.league?.toUpperCase() || 'Unknown',
                      ...
                      <SmartMoneyIndicator gameId={bet.id} homeTeam={bet.homeTeam} awayTeam={bet.awayTeam} />
