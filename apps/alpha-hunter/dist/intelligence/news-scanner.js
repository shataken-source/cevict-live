...
class NewsScanner {
    ...
    filterProfitOpportunities(news) {
    ...
    async analyzeNewsForOpportunities(news) {
        const dataPoints = [];
    ...
    analyzeSentiment(news) {
        ...
        let score = 0;
        bullishWords.forEach(word => { if (text.includes(word))
            score++; });
        bearishWords.forEach(word => { if (text.includes(word))
            score--; });
