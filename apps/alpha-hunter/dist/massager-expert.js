...
exports.MassagerExpertBot = void 0;
...
const massager_client_1 = require("./intelligence/massager-client");
...
╚══════════════════════════════════════════════════════════════════╝
     ...
     */
    async createHybridDataset() {
        console.log('📊 Creating hybrid dataset...');
        // Get real market data
        const realData = await data_aggregator_1.dataAggregator.getFullBriefing();
     ...
     */
    generateCrossCorrelations(realData, abstractData) {
        const correlations = [];
        // Fear & Greed vs Abstract momentum
        const fgMomentumCorr = this.calculateCorrelation(realData.market.fearGreedIndex.value, abstractData.filter(d => d.type === 'price_momentum').map(d => d.value));
     ...
     */
    calculateCorrelation(value, array) {
        if (array.length === 0)
            return 0;
     ...
     */
    async discoverPatterns(dataset) {
        if (!this.claude) {
  ...
  "potentialValue": "low|medium|high"
    ...
    heuristicDiscovery(dataset) {
        ...
        if (dataset.crossCorrelations?.length > 0) {
            const strongCorr = dataset.crossCorrelations.find((c) => c.significance === 'high');
            if (strongCorr) {
                discoveries.push({
     ...
     */
    async generateHypotheses(discoveries) {
        const hypotheses = [];
     ...
     */
    async runTrainingSessionWithTimeout() {
...
║  💡 HYPOTHESES TO TEST:                                          ║`);
     ...
     */
    async startLearning(intervalMinutes = 30) {
...
exports.MassagerExpertBot = MassagerExpertBot;
// Main
async function main() {
...
//# sourceMappingURL=massager-expert.js.map