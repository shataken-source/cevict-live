...
import { Opportunity, DataPoint, MarketData } from '../types';
...
interface CryptoOpportunity {
  type: 'prediction_market' | 'spot_trade' | 'arbitrage';
  symbol: string;
...
export class CryptoScanner {
  ...
  async scanMarkets(): Promise<Opportunity[]> {
    ...
    if (fearGreedOpp) opportunities.push(fearGreedOpp);
  ...
  private async getPrices(): Promise<CryptoPrice[]> {
    try {
      // CoinGecko API
  ...
  private async analyzeCrypto(crypto: CryptoPrice): Promise<Opportunity[]> {
    const opportunities: Opportunity[] = [];
  ...
  private async analyzeFearGreed(): Promise<Opportunity | null> {
    try {
      // Fear & Greed Index
  ...
  private async analyzeKalshiCrypto(): Promise<Opportunity[]> {
  ...
  private getSamplePrices(): CryptoPrice[] {
    return [
