 ...
 * Service list is driven by lib/config.ts getMonitoredServices() so new webapps are included when added to AI_PROJECTS.
 */

import { BotConfig, Issue, Severity, BotEvent } from './index'
import { getMonitoredServices } from '@/lib/config'
...
export class MonitorBot {
  ...
  /** Production endpoints to monitor – from config so new webapps are auto-included */
  private get SERVICES() {
    const extra = [
      { id: 'supabase', name: 'Database', url: 'https://nqkbqtiramecvmmpaxzk.supabase.co/rest/v1/' }
    ]
    return [...getMonitoredServices(), ...extra]
  }
  ...
  private async runHealthChecks() {
    ...
    for (const service of services) {
      if (!this.state.services.has(service.id)) {
        this.state.services.set(service.id, {
