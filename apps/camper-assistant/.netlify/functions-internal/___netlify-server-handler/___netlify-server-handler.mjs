...
} from '\var\task\apps\camper-assistant/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\apps\camper-assistant/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\apps\camper-assistant')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\apps\camper-assistant',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\apps\camper-assistant\.netlify\dist\run\handlers\server.js')
