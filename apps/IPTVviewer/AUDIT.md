...
**Screens:** TVHome → Channels → Player → EPG → Favorites → History → Settings → CatchUp → Recordings → QualitySettings → Devices (15 total, 14 in navigator)
**State:** Zustand store + AsyncStorage persistence
**Services:** 38 service files covering EPG, ad detection, VPN, Chromecast, recording, catch-up, stream quality, multi-device, weather, TMDB, TVMaze, and more
...
```
src/
├── screens/          # 15 screens (TVHome, Channels, Movies, Series, Player, EPG, Favorites, History, Settings, Pricing, CatchUp, Recordings, QualitySettings, Devices, + legacy Home)
...
### Now have UI screens:
- `CatchUpService.ts` → `CatchUpScreen.tsx` — 7-day catch-up browser
- `CloudDVRService.ts` → `RecordingsScreen.tsx` — DVR recording manager
- `StreamQualityService.ts` → `QualitySettingsScreen.tsx` + PlayerScreen overlay
- `MultiDeviceService.ts` → `DevicesScreen.tsx` — Device + session management

### Still no UI (dead code until wired to screens):
- `RecordingService.ts` — DVR recording (older service, superseded by CloudDVRService)
...
Prioritize `SleepTimerService` and `ChromecastService` — both are high-value, low-risk to add UI for.
