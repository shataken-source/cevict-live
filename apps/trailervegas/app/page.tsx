...
export default function HomePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error || 'Failed to create checkout session'); setLoading(false); return }
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 24px 60px' }}>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: '28px 28px 24px' }}>
          ...
          <div style={{ marginBottom: 20 }}>
            <label htmlFor="picks-file" style={{ display: 'block', fontSize: 10, color: C.textDim, marginBottom: 6 }}>Picks file (CSV or JSON)</label>
            <input
              id="picks-file"
              type="file"
        ...
        <div style={{ marginTop: 32 }}>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: '16px 20px' }}>
            {[
              { q: 'What sports are supported?', a: 'NFL, NBA, NHL, MLB, NCAAB, NCAAF — any game with public outcome data.' },
              { q: 'How are picks graded?', a: 'We match your picks against final game scores. Win/loss determined by moneyline result.' },
              { q: 'What if some games haven\'t been played yet?', a: 'Those picks will show as "pending" in your report.' },
              { q: 'Can I get a refund?', a: 'Reports are generated instantly. Contact support for issues.' },
            ].map((faq, i) => (
              <div key={i} style={{ paddingBottom: 12, marginBottom: 12, borderBottom: i < 3 ? `1px solid ${C.border}` : 'none' }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: C.textBright, marginBottom: 4 }}>{faq.q}</div>
                <div style={{ fontSize: 11, color: C.textDim, lineHeight: 1.5 }}>{faq.a}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 40, fontSize: 10, color: C.textDim }}>
          Powered by Cevict &bull; Data from public game outcomes &bull; Not gambling advice
        </div>
      </div>
    </div>
  )
