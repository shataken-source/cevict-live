...
function SearchLostPetsContent() {
  ...
  }, [filters, loadRecentPets]);

  // Keep local state in sync with URL querystring (?q=...)
  useEffect(() => {
    setSearchQuery(initialQuery);
  }, [initialQuery]);

  // Run searches when query or filters change
  useEffect(() => {
    if (searchQuery.trim()) {
  ...
  }, [loadRecentPets, performSearch, searchQuery]);
