...
export async function GET(_request: NextRequest, props: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (id && demoPetsById[id]) {
      return NextResponse.json({ success: true, pet: sanitizeContact(demoPetsById[id]), demo: true }, { status: 200 });
    }
    ...
    if (!res.ok) return NextResponse.json({ success: false, error: 'Pet not found' }, { status: 404 });
    ...
    return NextResponse.json({ success: true, pet: sanitizeContact(pet) }, { status: 200 });
  } catch (error: any) {
    console.error('[PET API] Error:', error);
    return (
      toServiceUnavailableResponse(error) ||
      NextResponse.json({ success: false, error: error.message || 'Failed to fetch pet' }, { status: 500 })
    );
  }
}
