...
export default function MyPetsPage() {
  ...
  useEffect(() => {
    ...
    if (savedSearch) {
      try {
        ...
        if (saved.value) {
          handleSearch(saved.type, saved.value, false);
        }
      } catch (e) {
        // Ignore parse errors
      }
  ...
  const handleSearch = async (type?: 'email' | 'phone' | 'name', value?: string, saveSearch: boolean = true) => {
    const searchTypeToUse = type || searchType;
    const searchValueToUse = value || searchValue;
    ...
    try {
      const response = await fetch(`/api/petreunion/find-my-pet?type=${searchTypeToUse}&value=${encodeURIComponent(searchValueToUse)}`);
      const data = await response.json();

      if (response.ok && data.success) {
        // Fetch detailed stats for each pet
        const petsWithStats = await Promise.all((data.pets || []).map(async (pet: Pet) => {
          try {
        ...
        if (petsWithStats.length === 0) {
          ...
          localStorage.setItem('petreunion_my_pets_search', JSON.stringify({
  ...
  const refreshPets = async () => {
    ...
    setRefreshing(true);
    await handleSearch(searchType, searchValue, false);
    setRefreshing(false);
  };
