  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (photoFile && photoFile.size > 0) {
        // Convert file to base64 for now (in production, upload to storage first)
    ...
    // Optional background matching (ignore failures)
    try {
      fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3007'}/api/petreunion/match-pets`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ petId: (newPet as any).id }),
      }).catch(() => { });
    } catch {
      // ignore
    }
