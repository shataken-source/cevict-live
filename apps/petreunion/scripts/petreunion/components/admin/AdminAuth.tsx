...
export default function AdminAuth({ children }: { children: React.ReactNode }) {
  ...
  // Cookie-based auth (set by /api/admin/login). Validate by calling an admin API.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl p-8">
          <div className="text-center mb-8">
            ...
            <div className="mt-2 text-xs text-slate-500">UI build: scripts-adminauth-2026-01-15</div>
          </div>
