...
export class ConnectionPool {
  ...
  async close(): Promise<void> {
    ...
    const activeConnections = this.connections.filter(c => c.inUse);
    if (activeConnections.length > 0) {
      console.log(`Waiting for ${activeConnections.length} active connections to finish...`);
      
      // Wait up to 10 seconds for connections to finish
      const startTime = Date.now();
      while (this.connections.some(c => c.inUse) && (Date.now() - startTime) < 10000) {
...
if (typeof process !== 'undefined') {
  const shutdown = async (signal: string) => {
    console.log(`\n${signal} received, closing connection pool...`);
    await closePool();
    process.exit(0);
  };
