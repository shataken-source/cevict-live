...
import { Loader2, Database, TrendingUp, CheckCircle2, AlertCircle, Play, BarChart3 } from 'lucide-react';
  ...
  const handlePopulate = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setResults({ error: data.error || 'Failed to populate database' });
      }
    } catch (error: any) {
  ...
  const handleBulkFacebookScrape = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setResults({ error: data.error || 'Failed to bulk scrape Facebook' });
      }
    } catch (error: any) {
      setResults({ error: error.message || 'Network error' });
    } finally {
  ...
  const handleBulkAllPlatforms = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setResults({ error: data.error || 'Failed to bulk scrape all platforms' });
      }
    } catch (error: any) {
      setResults({ error: error.message || 'Network error' });
    } finally {
