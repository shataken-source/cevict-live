...
try {
  ...
  console.log('[SCRAPER] Playwright not available, will use simple fetch');
}
  ...
  try {
    ...
    if (playwright) {
      try {
        ...
        console.log(`[PET SCRAPER BUBBLE] [${bubbleId}] Playwright launch failed: ${playwrightError.message}, falling back to fetch...`);
        browser = null;
        browserContext = null;
    ...
    // Puppeteer removed - using Playwright only or fetch fallback
    ...
    for (let page = 1; page <= maxPages; page++) {
      try {
        ...
        if (browser) {
          // Use Playwright for JavaScript-rendered content (puppeteer removed)
          try {
            let page: any;
            if (usingPlaywright && browserContext) {
              ...
              // This should not happen - puppeteer removed, only Playwright or fetch fallback
              throw new Error('Browser not available - should use fetch fallback');
            }
            ...
            await page.close();
            const browserType = usingPlaywright ? 'Playwright' : 'Fetch';
