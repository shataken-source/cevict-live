...
export async function POST(request: NextRequest) {
  try {
    ...
    const prognoResponse = await fetch(
      `${process.env.NEXT_PUBLIC_SITE_URL || 'https://petreunion-final.vercel.app'}/api/petreunion/progno-determine-search-areas`,
      {
        method: 'POST',
    ...
    const shelterSearches = searchAreas.map(async (area: any) => {
      try {
        ...
        const shelterResponse = await fetch(
          `${process.env.NEXT_PUBLIC_SITE_URL || 'https://petreunion-final.vercel.app'}/api/petreunion/shelter/search-from-zipcode`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    try {
      fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL || 'https://petreunion-final.vercel.app'}/api/petreunion/autonomous-fb-crawler`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
