...
// Use the existing FINNAI from wheretovacation which already has learning
// If it doesn't have logConversation, we'll add it
let finnAI: any;
try {
  finnAI = require('../lib/finnAI').default;
} catch {
  // Fallback - create simple learning wrapper
  finnAI = {
    getInstance: () => ({
...
const finnAIInstance = finnAI.getInstance();
...
export default function FinnConcierge({ userId }: { userId?: string }) {
  ...
  const handleSend = async () => {
      ...
      try {
        ...
        if (occasions.anniversaries.length > 0) {
          ...
          if (daysUntil <= 30 && daysUntil > 0) {
            proactiveMessage = `\n\n🎂 I see ${nextBd.name}'s birthday is coming up in ${daysUntil} days! Would you like to plan a special birthday celebration? I can help you find the perfect rental or activities! 🎉`;
          }
        }
      } catch (error) {
    ...
    if (userId || true) { // Log even for anonymous users
      ...
      try {
        // Try to use logConversation if available, otherwise use logInteraction
        if (finnAIInstance.logConversation) {
          await finnAIInstance.logConversation(
            userIdToUse,
            userInput,
            response.content,
            response.type
          );
        } else if (finnAIInstance.logInteraction) {
          await finnAIInstance.logInteraction(userIdToUse, {
            type: 'preference',
  ...
  const processMessage = async (message: string, userId?: string): Promise<{
    ...
    if (userId) {
      const recommendations = await finnAIInstance.getPersonalizedRecommendations(userId);
    ...
    try {
      ...
      if (boats.length > 0) {
        ...
        boats.slice(0, 2).forEach((boat: any, index: number) => {
          content += `${index + 1}. **${boat.name}** - ${boat.charter_type?.replace(/_/g, ' ') || 'Charter'}\n`;
          content += `   $${boat.price_per_hour}/hour | ${boat.capacity} guests | ${boat.location}\n\n`;
        });
      }
      ...
      if (rentals.length > 0) {
        ...
        rentals.slice(0, 2).forEach((rental: any, index: number) => {
          content += `${index + 1}. **${rental.name}** - ${rental.rental_type?.replace(/_/g, ' ') || 'Rental'}\n`;
          content += `   $${rental.price_per_night}/night | ${rental.max_guests} guests | ${rental.location}\n\n`;
        });
      }
