 ...
 * 
 * AI chatbot assistant for Where To Vacation
...
function extractIntent(message: string): string {
  ...
  if (lowerMessage.includes('rental') || lowerMessage.includes('accommodation')) return 'rental';
  if (lowerMessage.includes('activity') || lowerMessage.includes('things to do')) return 'activity';
...
Your role:
- Help customers find and book vacation rentals
- Answer questions about destinations, activities, and accommodations
- Provide information about pricing, availability, and booking
- Suggest activities and attractions
- Help with vacation planning

Guidelines:
- Be friendly, helpful, and professional
- Use vacation/travel terminology naturally
...
- Use emojis sparingly (🏖️ 🐟 ✈️)`;
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: `Hi! I'm Fishy, your AI assistant. I'm currently experiencing some technical difficulties, but I'm here to help with questions about Where To Vacation. Please try again in a moment, or contact our support team for immediate assistance.`,
        }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
