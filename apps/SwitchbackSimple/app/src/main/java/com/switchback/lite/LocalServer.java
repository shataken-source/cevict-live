...
public class LocalServer extends NanoHTTPD {
    ...
    public Response serve(IHTTPSession session) {
        ...
        if (remoteIp != null && !remoteIp.equals("127.0.0.1") && !remoteIp.equals("::1")
        ...
        try {
            ...
            if (isPrivateHost(host)) {
                ...
                return newFixedLengthResponse(Response.Status.FORBIDDEN, "text/plain",
                        "Proxy to private addresses not allowed");
            }
        } catch (Exception e) {
        ...
        for (int attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                conn.setRequestProperty("User-Agent",
                        "Mozilla/5.0 (Linux; Android 12; Streaming Box) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36");
                ...
                if (code >= 200 && code < 400) {
                    ...
                    // For live streams (no content-length or video/audio types), use chunked
                    // streaming
                    ...
                    // Many IPTV providers return non-standard HTTP codes (e.g. 513)
                    // but still include valid JSON data in the response body.
                    // Read the error stream fully and check if it contains JSON.
                    byte[] errBytes = null;
                    ...
                    // If error body looks like JSON, pass it through as 200
                    if (errBytes != null && errBytes.length > 2) {
                        String trimmed = new String(errBytes, "UTF-8").trim();
                        if (trimmed.startsWith("[") || trimmed.startsWith("{")) {
                            Log.i(TAG, "Proxy HTTP " + code + " but body is JSON (" + errBytes.length
                                    + " bytes) — passing through");
                            InputStream bodyStream = new java.io.ByteArrayInputStream(errBytes);
                            Response resp = newFixedLengthResponse(Response.Status.OK, "application/json", bodyStream,
                                    errBytes.length);
                            resp.addHeader("Access-Control-Allow-Origin", "*");
                            return resp;
                        }
                    ...
                    String errBody = errBytes != null ? new String(errBytes, "UTF-8") : "";
    ...
    /**
     * Rewrites an M3U8 playlist so every URI line (segment .ts, sub-playlist .m3u8,
     * etc.)
     * is converted to an absolute URL and then wrapped as:
     * http://localhost:8123/proxy?url=<encoded_absolute_url>
     *
     * This is required because HLS.js resolves segment URLs relative to the m3u8
     * base URL
     * and fetches them directly — which fails inside the Android WebView sandbox
     * since the
     * IPTV server is an external host. Routing everything through the local proxy
     * fixes it.
     */
    private String rewriteM3u8(String playlist, String baseUrl) {
        ...
        if (lastSlash > 8)
            base = base.substring(0, lastSlash + 1); // keep trailing slash
            ...
            if (trimmed.isEmpty() || trimmed.startsWith("#")) {
                ...
                if (out.contains("URI=\"")) {
                    ...
                    while (pos < out.length()) {
                        ...
                        if (start < 0) {
                            rewritten.append(out.substring(pos));
                            break;
                        }
    ...
    private boolean isPrivateHost(String host) {
        ...
        // Block private IP ranges by pattern (no DNS lookup — avoids hangs on Android
        // TV)
