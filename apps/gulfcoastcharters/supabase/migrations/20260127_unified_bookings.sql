...
CREATE TABLE IF NOT EXISTS public.unified_bookings (
  ...
  wtv_booking_id UUID, -- References bookings table in WTV database (if cross-DB) or NULL
  ...
  -- Note: References GCC's bookings table
  gcc_booking_id UUID, -- References bookings.id in GCC database
...
COMMENT ON COLUMN public.unified_bookings.wtv_booking_id IS 'Reference to WTV bookings.id (if cross-DB, may be NULL)';
COMMENT ON COLUMN public.unified_bookings.gcc_booking_id IS 'Reference to GCC bookings.id';
