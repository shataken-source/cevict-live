...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      // Random delay before request (0.5-2 seconds) to avoid pattern detection
      // Only for GET requests (POST requests to APIs shouldn't have random delays)
      ...
      // For POST requests (like Serper.dev API), use minimal headers
      // For GET requests (scraping), use browser-like headers
      let defaultHeaders: Record<string, string>;
      
      if (method === 'POST') {
        // API requests - minimal headers, let the caller specify what's needed
        defaultHeaders = {};
      } else {
        ...
        // Add Referer if provided (makes it look like we came from Google search)
        if (referer) {
      ...
      // Handle 403/429 errors - read response body to get more details
      if (res.status === 403 || res.status === 429) {
        ...
        // For API calls (POST), log the actual error from the API
        if (method === 'POST') {
          console.error(`[Web Search] API returned ${res.status}: ${errorBody.substring(0, 500)}`);
        }
        
        throw new Error(`HTTP ${res.status} ${res.statusText} - ${errorBody.substring(0, 200)}`);
      }
      ...
      // If it's a 403/429, don't retry - the site is blocking us
...
function extractLengthAndType(text: string): { length: number | null; boat_type: string | null } {
  ...
  const pattern1 = text.match(/(\d{2,3})[- ]*(?:ft|foot|'|feet)[\s-]+([A-Z][A-Za-z0-9\s-]{2,30}?)(?:\s|$|,|\.|powered|HP)/i);
  if (pattern1?.[1] && pattern1?.[2]) {
    ...
    if (Number.isFinite(len) && len >= 15 && len <= 200) {
      ...
      type = type.replace(/\s+(with|and|the|a|an|in|on|at|is|was|are|were|has|have|had|cabin|bathroom|bathroo)\s*$/i, '');
      
      // Filter out partial/incomplete boat types
      const typeLower = type.toLowerCase();
      if (type && type.length >= 3 && type.length <= 40
          ...
          && !typeLower.includes('is the') && !typeLower.includes('with a')) {
        boat_type = type;
      }
    }
  }
...
/**
 * Step 1: Search the web using SerpApi to find charter boat URLs
 */
async function searchWebForCharterBoats(target: number, filterState?: string | null): Promise<string[]> {
  ...
  try {
    ...
    // Limit to prevent timeouts - search first 3 cities, 3 queries
    ...
    for (const state of states.slice(0, 2)) {
      if (urls.length >= target * 2) break; // Get 2x URLs since some may fail to scrape
        ...
        for (const query of limitedQueries) {
          ...
          try {
            ...
            try {
              ...
              // Note: We don't include 'location' parameter because:
              // 1. The query already includes city and state (e.g., "charter boat Destin FL")
              // 2. SerpApi location format is strict and can cause 400 errors
              // 3. The 'gl' parameter (country) is sufficient for localization
            ...
            if (!data.organic_results) {
              ...
              // Check search_metadata for status
              if (data.search_metadata?.status) {
                console.log(`[Web Search] SerpApi status: ${data.search_metadata.status}`);
              }
            }
  ...
  return urls.slice(0, target * 2); // Return up to 2x target for scraping
}
...
async function scrapeBoatPage(url: string, defaultCity?: string, defaultState?: string): Promise<ScrapedBoat | null> {
  try {
    ...
    const html = await fetchWithRetry(url, { timeout: 15000 });
    console.log(`[ScrapeBoatPage] HTML length: ${html.length} chars`);
    
    // Extract meta description (often has key info)
    ...
    // Combine meta description, structured text, and full text for better extraction
    const combinedText = `${metaDescription} ${structuredText} ${fullText}`;
    ...
    if (!captain) captain = extractCaptainName(url, metaDescription);
    if (!captain) captain = extractCaptainName(url, fullText);
    
    let phone = extractPhone(structuredText);
    if (!phone) phone = extractPhone(metaDescription);
    ...
    // Do this BEFORE checking aggregator patterns so we catch "Charter&#039;s" correctly
    name = name
      .replace(/&amp;/g, '&')
      .replace(/&nbsp;/g, ' ')
      .replace(/&lt;/g, '<')
    ...
    // Try to extract location from URL or text
    ...
    if (captain) {
      const excludePhrases = ['will discuss', 'and crew', 'was great', 'is great', 'are great', 
                               'experiences', 'charters', 'fishing', 'boat', 'marina'];
      const captainLower = captain.toLowerCase();
      if (excludePhrases.some(phrase => captainLower.includes(phrase))) {
        validCaptain = null;
      }
      // Also check if it's a valid name format (2-3 words, capitalized)
      const words = captain.split(/\s+/);
    ...
    // Debug logging for manual URL mode
    const isManualUrl = false; // Will be set by caller context if needed
    if (url.includes('doubleacecharters') || url.includes('godaddysites')) {
      console.log('[Scrape Debug] Extraction results:', {
        name,
        captain,
        validCaptain,
        phone,
...
async function scrapeWebSearch(target: number, filterState?: string | null): Promise<ScrapedBoat[]> {
  ...
  try {
    // Step 1: Search for URLs using SerpApi
    ...
    // Step 2: Scrape each URL to extract boat details
...
/**
 * Verify request is authorized (from service role or admin)
 */
function verifyAuth(req: Request): boolean {
  ...
  // Allow if Authorization header matches service role key
  if (authHeader && authHeader.startsWith('Bearer ')) {
    const token = authHeader.substring(7);
    if (token === serviceKey) return true;
  }
  
  // Allow if apikey header matches service role key
  if (apiKey === serviceKey) return true;
  
  // In development, allow if service key is set (for testing)
...
serve(async (req) => {
  ...
  try {
    ...
    // Continue anyway - status update is best-effort
  }
  ...
  try {
    ...
    // Scrape sources with error handling (graceful degradation)
    ...
    if (wanted.has('thehulltruth')) {
      try {
        ...
        // Continue - don't fail entire run if one source fails
      }
    }
    ...
    // Web search using DuckDuckGo (searches entire web, finds new boats each run)
    ...
    // If all sources failed, still return success with empty results
    // This allows the UI to show the error messages
    ...
    // Log scraper run (best-effort, don't fail if this errors)
    try {
    ...
    // Create failure report if there are failures (best-effort)
    ...
    // Update scraper status (best-effort)
    try {
    ...
    // Create notification if new boats found (best-effort)
