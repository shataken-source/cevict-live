...
export async function createUnifiedReview(
  ...
  // Award loyalty points for the review (non-blocking)
  if (data) {
    awardReviewPoints(userId, data.id, reviewData.review_type, reviewData.platform).catch(err => {
      console.warn('Failed to award loyalty points for review (non-critical):', err);
    });
  }
...
/**
 * Award loyalty points for a review
 * Simple implementation: 10 points per review, bonus for detailed reviews
 */
async function awardReviewPoints(
  ...
  try {
    // Base points: 10 points for leaving a review
    let points = 10;
    
    // Bonus for detailed reviews (has title and content)
    // This will be checked after review is created, so we'll just give base points for now
    // Could enhance later to check if review has title + content
    ...
    const { error: txError } = await supabaseAdmin
      ...
      .insert({
        ...
        source_type: 'review',
        source_id: reviewId,
        description: `Points earned for leaving a ${reviewType} review`,
        metadata: {
    ...
    console.log(`✅ Awarded ${points} loyalty points to user ${userId} for review ${reviewId}`);
  } catch (error) {
    console.error('Error awarding review points:', error);
    // Don't throw - points are nice to have but shouldn't break review creation
  }
}
