/**
 * Message Board Points Helper
 * Awards points for message board activity
 */
...
export const MESSAGE_BOARD_POINTS = {
...
/**
 * Award points for message board activity
 */
export async function awardMessageBoardPoints(
  userId: string,
  action: 'create_post' | 'reply_to_post' | 'helpful_reply' | 'first_post' | 'share_post',
  metadata?: Record<string, any>
): Promise<{ success: boolean; points: number; totalPoints: number; error?: string }> {
  if (!supabaseAdmin) {
    return { success: false, points: 0, totalPoints: 0, error: 'Database not configured' };
  }
  ...
  try {
    ...
    switch (action) {
      ...
      case 'share_post':
        ...
        break;
      default:
        return { success: false, points: 0, totalPoints: 0, error: 'Invalid action' };
    }
      ...
      if (count === 0) {
        ...
        await supabaseAdmin
          ...
          .insert({
            ...
            source_id: metadata?.post_id || null,
            description: 'First post bonus!',
            metadata: { ...metadata, is_first_post: true },
          });
      }
    }
    ...
    const { error: txError } = await supabaseAdmin
      ...
      .insert({
        ...
        source_id: metadata?.post_id || metadata?.reply_id || null,
    ...
    if (txError) {
      ...
      return { success: false, points: 0, totalPoints: 0, error: txError.message };
    }
    ...
    console.log('[Points] Calculated total:', calculatedTotal, 'Shared users total:', sharedUser?.total_points, 'Returning:', totalPoints);
    ...
    return { success: false, points: 0, totalPoints: 0, error: error.message };
  }
}
