 ...
 * POST /api/unified-bookings/create
 */
...
export default async function handler(
  ...
  try {
    ...
    if (!bookingData || !bookingData.booking_type) {
      return res.status(400).json({ error: 'bookingData with booking_type required' });
    }
    ...
    if (!booking) {
      return res.status(500).json({ error: 'Failed to create unified booking' });
    }
