  ...
  const fetchAccount = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setStatusPageSlug(d.status_page_slug ?? '');
      }
    } catch {
      // ignore
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        if (!res.ok) throw new Error(data.error || data.details || 'Failed to create website');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white p-4 md:p-8" style={{ fontSize: 'clamp(14px, 1.5vw, 18px)' }}>
      <div className="max-w-7xl mx-auto">
        ...
        {/* Status page slug */}
        <div className="bg-slate-800/50 border border-blue-500/30 rounded-xl p-4 md:p-6 mb-6 md:mb-8">
          <h2 className="text-xl md:text-2xl font-bold mb-4">Public status page</h2>
          <p className="text-gray-400 text-sm mb-4">
            Set a slug to get a public URL like <strong>/status/your-slug</strong> (e.g. for cevict.ai/monitor/status/your-slug).
          </p>
          <div className="flex flex-wrap items-center gap-3">
            <input
              type="text"
              value={statusPageSlug}
              onChange={(e) => setStatusPageSlug(e.target.value.replace(/[^a-z0-9-_]/gi, '').toLowerCase())}
              className="bg-slate-900 border border-slate-700 rounded px-4 py-2 text-white w-48"
            ...
            <button
              onClick={async () => {
                ...
                try {
                  const res = await fetch('/api/admin/account', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ status_page_slug: statusPageSlug }),
                  });
                  const data = await res.json();
                  if (!res.ok) throw new Error(data.error || 'Failed to save');
                  setStatusPageSlug(data.status_page_slug ?? '');
                } catch (err: any) {
              ...
              disabled={saving}
              className="bg-blue-600 hover:bg-blue-500 disabled:bg-gray-600 px-4 py-2 rounded-lg font-semibold"
            >
