...
import { Activity, Globe, MessageSquare, ChevronRight, RefreshCw } from 'lucide-react';
...
export default function MonitorDashboard() {
  ...
  const [subscription, setSubscription] = useState<{ plan: string; limit: number } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/subscription')
      ...
      .then(d => d?.ok && (d.plan != null) && (d.limit != null) ? { plan: d.plan, limit: d.limit } : null)
  ...
  const fetchWebsites = async () => {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 15000); // 15s timeout so we don't hang forever
    try {
      const res = await fetch('/api/websites', { signal: controller.signal });
      ...
      if (res.ok) {
        ...
        setLoadError('Dashboard failed to load. Try again.');
        setWebsites([]);
      }
    } catch (error) {
      clearTimeout(timeoutId);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white p-4 md:p-8" style={{ fontSize: 'clamp(14px, 1.5vw, 18px)' }}>
      <div className="max-w-[1920px] mx-auto">
        ...
        {/* Load error */}
        {loadError && (
          <div className="mb-6 bg-amber-900/30 border border-amber-500/50 rounded-xl p-6 text-center">
            <p className="text-amber-200 mb-4">{loadError}</p>
            <button
              onClick={() => { setLoading(true); fetchWebsites(); }}
              className="inline-block bg-amber-600 hover:bg-amber-500 px-6 py-3 rounded-lg font-semibold"
            >
              Retry
            </button>
          </div>
        ...
        {/* Onboarding: first website */}
        {!loadError && websites.length === 0 && (
          <div className="mb-6 bg-blue-900/30 border border-blue-500/50 rounded-xl p-6 text-center">
            <h2 className="text-xl font-bold mb-2">Add your first website</h2>
            <p className="text-gray-400 mb-4">Start monitoring uptime, response time, and visitors in one place.</p>
            <Link
              href="/admin"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            ...
            return (
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  {/* Last checked */}
                  {siteStats?.uptime?.lastCheckedAt != null && (
                    <div className="flex justify-between items-center text-gray-400 text-xs">
                      <span>Last checked</span>
                      <span>{formatLastChecked(siteStats.uptime.lastCheckedAt)}</span>
                    </div>
                  )}
          ...
          {/* Add Website Card or Upgrade CTA */}
          {subscription && websites.length >= subscription.limit ? (
            <Link
              href="/landing"
              className="bg-amber-900/20 border-2 border-dashed border-amber-500/50 rounded-xl p-4 md:p-6 flex items-center justify-center hover:border-amber-400 transition-all min-h-[200px]"
            >
              <div className="text-center">
                <Globe className="w-8 h-8 md:w-12 md:h-12 mx-auto mb-2 text-amber-400" />
                <div className="text-amber-200 text-sm md:text-base font-medium">Plan limit reached ({subscription.limit} sites)</div>
                <div className="text-gray-400 text-xs mt-1">Upgrade to add more</div>
              </div>
            </Link>
          ) : (
            <Link
