...
# Optional: SMS alerts via Sinch (if unset, alerts are logged only)
SINCH_SERVICE_PLAN_ID=your_service_plan_id
SINCH_API_TOKEN=your_api_token
SINCH_FROM_NUMBER=+1234567890
SINCH_REGION=us

# Optional: Email alerts via Resend (if unset, email alerts are logged only)
RESEND_API_KEY=re_xxxx
# RESEND_FROM=Website Monitor <alerts@yourdomain.com>  # optional, default onboarding@resend.dev
```
...
Set `SINCH_SERVICE_PLAN_ID`, `SINCH_API_TOKEN`, and `SINCH_FROM_NUMBER` in `.env.local` to send real SMS; otherwise alerts are logged only.
...
Before going live: see `docs/PRODUCTION_READINESS_AUDIT.md`. Critical items: secure `/api/alerts/sms` and `/api/bots/status`, Stripe Live keys, Clerk production config.
